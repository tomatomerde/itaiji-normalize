...
再実行は安全である。GitHub Release も同じで、そのタグの Release が既にあれば `gh release view`
で検知してそのまま残す。
...
Actions タブ → **Release** → **Run workflow**、`dry_run` は `true`、`cut_release` は `false`
のまま。`npm publish` 以外のすべてが、そのときディスク上にあるバージョンに対して同一に
実行される。タグがないため、バージョンと CHANGELOG のガードは適用されない — だからこそ
バージョンと dist-tag がサマリーに印字される。間違っていれば、実害が出る前に見えるように。
...
ページはもう最新ではない挙動を、最新であるかのように表示し続ける。

忘れたときに気づける仕掛けが2つある。`demo/build.sh` はレジストリの `latest` と
食い違うと warning を出す。ただし**リリースは `demo/**` を触らないので、paths
フィルタのせいで `pages.yml` 自体が走らず、その warning は誰にも見えない**。
そこを埋めるために `pages.yml` に**週次スケジュールの `pin-freshness` ジョブ**を
置いてあり、固定とレジストリの `latest` が違えば赤くなる。**気づくまで最大1週間**なので、
仕掛けに頼らずここで上げるのが本筋。詳細は [`demo/README.md`](../demo/README.md)。
